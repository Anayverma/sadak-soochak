      ...
      if (response.ok) {
        ...
        if (secondResponse.ok) {
          ...
          setResultImageUrl(secondData.resultImageUrl);
            console.log(secondData.resultImageUrl)
          try{
            ...
            const finalResponse = await fetch("/api/savePost", {
              ...
              body: JSON.stringify({
                name,
                location,
                image: secondData.resultImageUrl,
              }),
            });
            if (finalResponse.ok) {
  ...
  return (
    <main className="container mx-auto p-4 relative mainhead ">
      ...
      <form onSubmit={handleSubmit} className="max-w-lg mx-auto">
        ...
        {cloudinaryUrl && (
          <Image cloudName={cloudName} publicId={cloudinaryUrl} />
        )}
        <button
          type="submit"
      ...
      {displayMessage && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-green-500 text-white p-4 rounded">
          ...
          {resultImageUrl && (
            <Image cloudName={cloudName} publicId={resultImageUrl} />
          )}
        </div>
      )}
    </main>
